public class Day03 : PuzzleDay
{
    public override void Part01()
    {
        var result = Input.Split("\n").Aggregate(0, (i, s) =>
        {
            ...
            var compartment1 = s[..midPoint];
            var compartment2 = s[midPoint..];
            i += compartment2.ToHashSet().Where(c => compartment1.IndexOf(c) >= 0).Sum(c => GetPriority(c));
            return i;
        });
    ...
    public override void Part02()
    {
        ...
        var result = 0;
        for (var i = 0; i < parsedInput.Length; i+=3)
        {
            var elf1 = parsedInput[i];
            var elf2 = parsedInput[i+1];
            var elf3 = parsedInput[i+2];
            var common = elf1.Intersect(elf2.ToHashSet()).Intersect(elf3.ToHashSet()).Single();
    ...
    protected override string InputExample => @"vJrwpWtwJgWrhcsFMMfFFhFp
jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL
PmmdzqPrVvPwwTWBwg
wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn
ttgJtRGJQctTZtZT
CrZsJsPPZsGzwwsLwLmpwMDw";
    protected override string Input => @"vGFhvGvvSdfwqhqvmCPnlFPnCNPcCFcWcr
